...
function Phone_ver({ route }) {
    ...
    const ip = await AsyncStorage.getItem('@Data_Ip')
      ...
      const requestOptions = {
          ...
          body:JSON.stringify({
            ...
            "phone":  route.params.obj.phone,

           
          })
      };
      
        fetch('http://'+ip_add['ip']+'/api/Customer/', requestOptions)
        .then((response) => response.json())
        .catch((error)=> {Alert.alert("Please Check Detials ")})
        Alert.alert("Congraulation ! account is Done Signup")
  ...
  const send_otp =async () => {
    ...
    fetch('http:/'+ip_add['ip']+'/api/Otp/'+route.params.Phone+'/'+seq, {
       method: 'GET'
    })
    .then((response) => response.json())
    .then((responseJson) => {
      // setSearch(responseJson)
       if (responseJson['statusCode']==200){
        ...
        Alert.alert("Please Check Otp Send Into Your Number"+route.params.Phone)
       }
       else{
        //  console.log(seq)
  ...
  const Check_otp = () => {
    ...
    if (value.length <= 3) {
      ...
      console.log(value)
      // const obj = {username: route.params.obj.username,age:route.params.obj.age ,password: route.params.obj.password,weight:route.params.obj.weight ,height: route.params.obj.height,email:route.params.obj.email,allergies:route.params.obj.allergies,phone:route.params.obj.phone};
      Send_data()
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <LottieView style={{ height: 200, marginLeft: 42, marginTop: 10 }} source={require('../assets/75250-sms-animation.json')} autoPlay />
      ...
      <View style={{ flex: 1, marginLeft: 50, marginRight: 70 }}>
        <TouchableOpacity
